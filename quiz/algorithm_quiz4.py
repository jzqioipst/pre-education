...
def greedy(amount, coins):
    ...
    for coin in coins:
        count = amount // coin
        info_lst.append(f"{coin}원 동전 {count}개")
...
amount_of_money = int(input("액수입력 : "))
kind_of_coins = sorted(list(map(int, input("동전의 종류 : ").split())), reverse=True)

print(greedy(amount_of_money, kind_of_coins))
